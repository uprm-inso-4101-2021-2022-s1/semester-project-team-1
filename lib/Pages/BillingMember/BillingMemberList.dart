...
class BillingMemberList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (memberClass == null) {
      return Text(
        "List is empty",
        style: TextStyle(color: Colors.white, fontSize: 25, fontFamily: 'iOS'),
      );
    } else {
      return Container(
              ...
              return Card(
                  ...
                  child: ListTile(
                    tileColor: (() {
                      ...
                      if (memberClass[index].status == "Deudor") {

                        return Colors.red;

                      } else if (memberClass[index].status == "Not Active") {

                        return Colors.red[300];

                      } else if (memberClass[index].inquiry != "" && memberClass[index].inquiry != null && memberClass[index].inquiry != "empty" ) {
                       
                        return Colors.blue[300];
                      
                      }  else if (DateTime.parse(memberClass[index].nextPayDate).isBefore(DateTime.now())) {
                    ...
                    leading: CircleAvatar(radius: 30,backgroundImage: NetworkImage("https://raw.githubusercontent.com/Jankeelol/jankeelol.github.io/main/img/AcropolisFitnessAssets/members/" +memberClass[index].memberID +".jpg"), //memberClass[index].imageURL), // no matter how big it is, it won't overflow
),
                    title: Text(memberClass[index].firstName +" " +memberClass[index].lastName,style: TextStyle(color: Colors.black, fontFamily: 'iOS'),),
                    subtitle: Text(memberClass[index].memberID),
                    trailing: Text(memberClass[index].payAmount +"\n" +newDateFormat.format(DateTime.parse(memberClass[index].nextPayDate)).toString(),textAlign: TextAlign.right,),
                    onTap: () {
